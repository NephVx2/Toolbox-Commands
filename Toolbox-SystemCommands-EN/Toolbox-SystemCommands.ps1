...
.NOTES
    ...
    v2.5.5: fixed a visible PowerShell console window flashing behind the GUI
    when launched from a desktop shortcut using pwsh.exe (or powershell.exe)
    with -WindowStyle Hidden, without "Run as administrator". -WindowStyle
    Hidden only applies to that first, non-elevated process - the
    auto-elevation block then launches a SECOND, separate process (powershell.exe
    -Verb RunAs) to actually run the script elevated, and that relaunch had no
    way to inherit or request a hidden window of its own, so the elevated
    Windows PowerShell 5.1 window it opened stayed visible behind the toolbox
    for the rest of the session. Not reproducible via right-click "Run as
    administrator" on the shortcut, since the process is already elevated in
    that case and this second launch never happens - which is what made the
    two launch methods behave differently. First attempted fix (adding
    -WindowStyle Hidden directly to that Start-Process -Verb RunAs call)
    turned out to be an unreliable pairing that could silently prevent the
    elevated process from launching at all (UAC prompt accepted, but no
    window ever appeared) - reverted. Fixed properly instead by hiding the
    console window via the Win32 API (GetConsoleWindow/ShowWindow) once the
    elevated instance is already running, right after the admin check passes
    - works regardless of which of the three launch paths (auto-elevation
    relaunch, a hidden shortcut, or "Run as administrator") got the process
    there.
#>
...
if (-not $SelfTest) {
    ...
    # Hides this process's own console window, regardless of how this elevated instance
    # was launched (the relaunch above, a desktop shortcut's own -WindowStyle Hidden, or
    # "Run as administrator"). Done via the Win32 API directly, once already running,
    # rather than via Start-Process -WindowStyle Hidden on the relaunch above: that
    # combination (-Verb RunAs together with -WindowStyle Hidden) is an unreliable pairing
    # that can silently prevent the elevated process from launching at all (confirmed: UAC
    # prompt accepted, but no window ever appears) - hiding the console after the fact
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA9rWTAOddGVhoA
# j/gfv3BmZp2w16ntr8gM0f/oeIrEf6CCAygwggMkMIICDKADAgECAhB6X4r8AlBU
...
# ARUwLwYJKoZIhvcNAQkEMSIEIBqF/vpaSuKRxxFFyxPBBgJG1mVK0zQWg4IgLutX
# UKWfMA0GCSqGSIb3DQEBAQUABIIBAIx0YCdpOFeZsvkahhzpsIbLxJTwmmVPBHDt
# 7Owj/NXv0HKPnD8C+8cC+/4h3Z1vNFkAnOp1L7TV9etzpTRUbrYcCsryJbl4Zh5u
# LtBsQhCZrWxoVJKq0oF3USPUR8cvdQHZxtnTcxS2/5cpApp1mGMkBd8xqVp4nhcs
# b6j4cKzQjSRQ5hF6uysjwEwMK3MDz+tiGQUSryY49Td6ju80nC1L51hzotssAGfm
# ifaUYL9nkWe4l/lISN+2Gwz6cjoiQDrqeAwvhIhEWAWm9GiTTl2VX3qpBep4EhDC
# TzhGE9agSFjyJhf4vkIQnSi7rEMSuPbGbd6ELavEJdwrtNuuzV0=
