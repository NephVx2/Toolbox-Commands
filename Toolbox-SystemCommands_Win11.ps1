...
function global:Export-History {
    ...
    # PC/Utilisateur ne varient quasiment jamais d'une ligne a l'autre (meme machine) ->
    # affiches une seule fois dans l'entete plutot que repetes sur chaque ligne, pour
    # liberer de la largeur pour la colonne Detail qui en a besoin.
    $derniereEntree = $rows[$rows.Count - 1]

    $grouped = $rows | Group-Object { ($_.Date -split ' ')[0] }

    $htmlRows = foreach ($grp in $grouped) {
        "<tr class=`"daysep`"><td colspan=`"5`">$($grp.Name)</td></tr>"
        foreach ($r in $grp.Group) {
            ...
            # majorite des lignes - retire pour la lisibilite, il n'apporte rien de
            # nouveau a chaque repetition.
            ...
            "<tr class=`"$rowClass`"><td>$($r.Date)</td><td>$($r.Win)</td><td>$($r.PID)</td><td>$titreHtml</td><td>$detailHtml</td></tr>"
        }
    }
...
<style>
  ...
  .searchbar input { width:320px; padding:8px 12px; background:#242428; border:1px solid #3c3c41; color:#e5e5e7; border-radius:4px; font-size:13px; }
  .searchbar input:focus { outline:none; border-color:#00c8ff; }
  ...
  tr.row:hover { background:#242428; }
  tr.sensible { background: rgba(255,190,90,0.06); }
  tr.daysep td { background:#0f0f11; color:#00c8ff; font-weight:bold; font-size:12px; text-transform:uppercase; letter-spacing:0.05em; padding-top:14px; }
  .label-sensible { color:#ffbe5a; font-weight:600; }
  code { color:#ffbe5a; font-family: Consolas, monospace; font-size:12px; word-break:break-all; }
  details summary { cursor:pointer; color:#00c8ff; font-size:12px; }
  details[open] summary { margin-bottom:6px; }
</style>
</head>
<body>
  <h1>Historique - Toolbox Commandes Systeme</h1>
  <div class="meta">Genere le $(Get-Date -Format "dd/MM/yyyy HH:mm:ss") - $($rows.Count) entree(s) - Machine : $($derniereEntree.PC) - Utilisateur : $($derniereEntree.User)</div>
  <div class="searchbar"><input type="text" id="searchBox" placeholder="Filtrer (nom de commande, date, PID...)" onkeyup="filterHistory()"></div>
  <table>
    <tr><th>Date</th><th>Windows</th><th>PID</th><th>Commande</th><th>Detail</th></tr>
    $($htmlRows -join "`n")
  </table>
<script>
function filterHistory() {
  var filtre = document.getElementById('searchBox').value.toLowerCase();
  var lignes = document.querySelectorAll('tr.row');
...
# ============================================================
# REGION: SELFTEST
# 39 assertions sur l'integrite du catalogue de commandes et les
