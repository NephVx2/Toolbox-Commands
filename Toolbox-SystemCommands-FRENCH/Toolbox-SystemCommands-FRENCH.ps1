...
.NOTES
    ...
    v2.5.5 : script et catalogues renommes pour la publication GitHub, ou la version
    francaise vit desormais dans son propre dossier (Toolbox-SystemCommands-FRENCH/),
    separe du dossier de la version anglaise. Toolbox-SystemCommands_Win11.ps1 ->
    Toolbox-SystemCommands-FRENCH.ps1, Commands.psd1 -> Commands-FRENCH.psd1,
    Commands.Personnel.psd1 -> Commands_Personnel-FRENCH.psd1. Chemins de chargement
    ($global:catalogPath, $global:personalCatalogPath), messages d'erreur affiches a
    l'utilisateur, et libelles des assertions -SelfTest concernes mis a jour en
    consequence - un renommage des fichiers sans ce correctif aurait laisse le script
    chercher "Commands.psd1" a cote de lui et echouer au chargement du catalogue avec le
    nouveau nom de fichier.

    v2.5.6 : correction d'une fenetre console PowerShell visible clignotant derriere
    l'interface graphique lors d'un lancement depuis un raccourci bureau utilisant
    pwsh.exe (ou powershell.exe) avec -WindowStyle Hidden, sans passer par "Executer en
    tant qu'administrateur". -WindowStyle Hidden ne s'applique qu'a ce premier processus
    non eleve - le bloc d'auto-elevation lance ensuite un SECOND processus distinct
    (powershell.exe -Verb RunAs) pour reellement executer le script en eleve, et ce
    relancement n'avait aucun moyen d'heriter ou de demander sa propre fenetre cachee,
    donc la fenetre Windows PowerShell 5.1 elevee qu'il ouvrait restait visible derriere
    la toolbox pour le reste de la session. Non reproductible via clic droit "Executer
    en tant qu'administrateur" sur le raccourci, puisque le processus est deja eleve
    dans ce cas et que ce second lancement n'a jamais lieu - ce qui expliquait la
    difference de comportement entre les deux methodes de lancement. Premiere tentative
    de correction (ajout de -WindowStyle Hidden directement sur cet appel Start-Process
    -Verb RunAs) s'est averee etre un assemblage peu fiable pouvant empecher
    silencieusement le processus eleve de se lancer du tout (prompt UAC accepte, mais
    aucune fenetre n'apparaissait jamais) - annulee. Corrige proprement a la place en
    masquant la fenetre console via l'API Win32 (GetConsoleWindow/ShowWindow) une fois
    l'instance elevee deja en cours d'execution, juste apres la verification des droits
    admin - fonctionne quel que soit celui des trois chemins de lancement (relancement
    par auto-elevation, raccourci cache, ou "Executer en tant qu'administrateur") qui a
    amene le processus jusque-la.
#>
...
if (-not $SelfTest) {
    ...
    # Masque la fenetre console de ce processus, quelle que soit la maniere dont cette
    # instance elevee a ete lancee (le relancement ci-dessus, un raccourci bureau avec son
    # propre -WindowStyle Hidden, ou "Executer en tant qu'administrateur"). Fait via l'API
    # Win32 directement, une fois deja en cours d'execution, plutot que via Start-Process
    # -WindowStyle Hidden sur le relancement ci-dessus : cette combinaison (-Verb RunAs
    # associe a -WindowStyle Hidden) est un assemblage peu fiable qui peut empecher
    # silencieusement le processus eleve de se lancer du tout (confirme : prompt UAC
    # accepte, mais aucune fenetre n'apparait jamais) - masquer la console apres coup
    # contourne entierement ce probleme.
    Add-Type -Name Window -Namespace ToolboxConsole -MemberDefinition '
        [DllImport("kernel32.dll")]
        public static extern IntPtr GetConsoleWindow();
        [DllImport("user32.dll")]
        public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow);
    '
    $consoleHandle = [ToolboxConsole.Window]::GetConsoleWindow()
    if ($consoleHandle -ne [IntPtr]::Zero) {
        [void][ToolboxConsole.Window]::ShowWindow($consoleHandle, 0)  # SW_HIDE
    }
}
# ============================================================
...
# ------------------------------------------------------------
# Catalogue des commandes - charge depuis Commands-FRENCH.psd1 (doit se trouver
# dans le meme dossier que ce script). Voir Commands-FRENCH.psd1 pour le format
# et les conventions (notamment : toujours des guillemets simples pour Cmd).
# ------------------------------------------------------------
$global:catalogPath = Join-Path $PSScriptRoot "Commands-FRENCH.psd1"
...
# de savoir quelles lignes de l'historique surligner sans devoir re-parser Commands-FRENCH.psd1.
$global:confirmLabels = [System.Collections.Generic.HashSet[string]]::new()

if (-not (Test-Path $global:catalogPath)) {
    $global:catalogLoadError = "Fichier Commands-FRENCH.psd1 introuvable a cote du script (chemin attendu : $global:catalogPath)."
} else {
    try {
        ...
        $global:catalogLoadError = "Echec du chargement de Commands-FRENCH.psd1 : $($_.Exception.Message)"
    }
}

# ------------------------------------------------------------
# Catalogue PERSONNEL, optionnel - Commands_Personnel-FRENCH.psd1, meme dossier, meme format
# que Commands-FRENCH.psd1. Contrairement au catalogue principal, celui-ci n'est PAS concu pour
# etre partage entre machines (reglages/decisions propres a une machine precise). S'il est
# absent, rien ne se passe : aucune erreur, le catalogue principal fonctionne normalement seul -
# c'est ce qui garantit que Commands-FRENCH.psd1 + le script restent copiables tels quels sur une
...
$global:personalCatalogPath = Join-Path $PSScriptRoot "Commands_Personnel-FRENCH.psd1"
...
if (Test-Path $global:personalCatalogPath) {
    try {
        ...
        $global:personalCatalogLoadError = "Echec du chargement de Commands_Personnel-FRENCH.psd1 : $($_.Exception.Message)"
    }
}
...
if ($global:personalCatalogLoadError -and -not $SelfTest) {
    # Non bloquant expres : Commands_Personnel-FRENCH.psd1 est optionnel, une erreur dessus ne doit
    # jamais empecher le catalogue principal (portable) de fonctionner normalement.
    [System.Windows.Forms.MessageBox]::Show(
        "$($global:personalCatalogLoadError)`n`nLe catalogue principal (Commands-FRENCH.psd1) reste charge normalement - seule la categorie personnelle est absente.",
...
if ($SelfTest) {
    ...
    # Commands-FRENCH.psd1 (simple chaine), ni ce -SelfTest sans ces deux assertions dediees - l'echec
    ...
    # --- Catalogue externe (Commands-FRENCH.psd1) ---
    Test-Assertion "Fichier Commands-FRENCH.psd1 present a cote du script" (Test-Path $global:catalogPath)
    Test-Assertion "Au moins une commande possede un champ Help (bouton '?')" (($allCommands | Where-Object { $_.Help }).Count -gt 0)
    Test-Assertion "Commands-FRENCH.psd1 charge sans erreur" (-not $global:catalogLoadError)
    Test-Assertion "Commands_Personnel-FRENCH.psd1 absent ou charge sans erreur (optionnel)" (-not $global:personalCatalogLoadError)

    # Regression : si une categorie de Commands_Personnel-FRENCH.psd1 porte le meme Name qu'une
    # categorie de Commands-FRENCH.psd1, le chargement sequentiel ($commandGroups[$cat.Name] = ...)
    ...
    Test-Assertion "Aucune categorie de Commands_Personnel-FRENCH.psd1 n'ecrase une categorie de Commands-FRENCH.psd1 (regression collision silencieuse)" ($categoriesEnCollision.Count -eq 0)
    ...
    Test-Assertion "Tous les champs Cmd de Commands-FRENCH.psd1 sont en guillemets simples (regression)" (-not $cmdEnGuillemetsDoubles)
    ...
    Test-Assertion 'Aucun $ non echappe dans les champs Desc/Help de Commands-FRENCH.psd1 (regression interpolation)' (-not $descHelpAvecDollarNonEchappe)
...
foreach ($group in $commandGroups.Keys) {
    ...
    foreach ($item in $commandGroups[$group]) {
        ...
        # commande possede un champ Help dans Commands-FRENCH.psd1 - sinon grise et desactive.
        $helpCopy = $item.Help
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCrByPWxaUyiyi/
# yA84ylzBikaiVeBHEl5LQUnxHnSTp6CCAygwggMkMIICDKADAgECAhB6X4r8AlBU
...
# ARUwLwYJKoZIhvcNAQkEMSIEIBX4vLl9F/hG85EkAqo13ishXhza1bjlhoG2gSM8
# isakMA0GCSqGSIb3DQEBAQUABIIBAFFijPdM31Ccp4JWfOU1EHv7MTd9UUY7FCQy
# g8GXzCOgQeNUh7jV+/L6ZFeH9HnvEvM7xLEXYnYGnyB3kv1ATEWoevFZEyzI99P9
# 0YBNpbxFB3hA6ov6yB6xN2RPjlN2kNghEkqJvzKuuZLbLPtwcFCYwpu1zSZQgPXB
# 4QZ7RSc1dh34dVRUWb0q1IkC+WHUOVLRWP7u/5HV5lEH98knD5YauP51BSUOlhQ3
# sP7D5M63Ejos0b/idSRQ4CkUiZyGPSnsHgfRDm5DweFLVN71w9Fw+o12n3QXk3LL
# zQkEopktOTHlBnvczuQAM0L4GT7n8iK8hL034LBD2ygouvFEKJE=
