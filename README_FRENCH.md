...
- [Catalogue personnel (commandes par machine)](#catalogue-personnel-commandes-par-machine)
- [Prerequis](#prerequis)
...
Deux fichiers compagnons accompagnent le script et doivent rester dans le meme dossier :

- **`Commands.psd1`** — le catalogue de commandes portable (145 commandes, destine a etre identique sur toutes vos machines)
- **`Commands.Personnel.psd1`** *(optionnel)* — un catalogue propre a une machine pour des commandes qui n'ont pas leur place dans le catalogue partage (voir [Catalogue personnel](#catalogue-personnel-commandes-par-machine))
...
## Catalogue personnel (commandes par machine)

`Commands.Personnel.psd1` est un second catalogue **optionnel**, meme dossier et meme format que `Commands.psd1`, mais **non** destine a etre partage ou synchronise entre machines — un endroit pour des commandes qui n'ont de sens que sur une machine precise (ex : gerer une tache planifiee qui n'existe que la).

- Absent → aucun impact, aucun avertissement ; le catalogue principal se charge et fonctionne normalement seul.
- Present mais invalide → un avertissement non bloquant s'affiche ; le catalogue principal continue de se charger normalement.
- Un nom de categorie qui entre en collision avec une categorie deja presente dans le catalogue principal **l'ecrase silencieusement** — une assertion dediee du self-test verifie cette collision specifique (comparaison des noms de categories des deux catalogues deja charges en memoire) pour qu'elle soit detectee avant de se manifester par une categorie du catalogue principal mysterieusement incomplete dans l'interface.
...
Ajouter une commande signifie editer directement `Commands.psd1` (ou `Commands.Personnel.psd1` pour quelque chose de propre a une machine) — il n'y a volontairement pas d'editeur integre a l'application, pour garder le catalogue en texte brut, facile a comparer, versionner et synchroniser entre machines.
...
1. **Distribuer les trois fichiers ensemble** : `Toolbox-SystemCommands_Win11.ps1`, `Commands.psd1`, et optionnellement un `Commands.Personnel.psd1` propre a la machine si necessaire. Le script et `Commands.psd1` doivent rester dans le meme dossier.
...
3. **Executer `-SelfTest` en premier** sur chaque machine — aucun droit admin necessaire, l'interface ne s'ouvre jamais, sans risque a lancer avant de decider de la suite. Un code de sortie `1` signifie que quelque chose dans le catalogue (ou une collision de catalogue personnel) necessite votre attention avant utilisation.
...
5. Garder `Commands.psd1` identique sur toutes les machines (c'est son but) ; utiliser `Commands.Personnel.psd1` localement sur toute machine ayant besoin de commandes que les autres ne doivent pas avoir.
...
<details>
<summary><strong>Toute une categorie semble avoir des commandes manquantes</strong></summary>

Si vous utilisez `Commands.Personnel.psd1`, verifier si l'un de ses noms de categorie correspond exactement a un nom de categorie du `Commands.psd1` principal — le catalogue personnel se charge *apres* le principal et ecrase silencieusement toute categorie portant le meme nom. `-SelfTest` a une assertion dediee pour cette collision ; renommer la categorie personnelle en quelque chose d'unique a la place.
</details>
